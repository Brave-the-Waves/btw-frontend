        ...
        <>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="lg:col-span-3">
                ...
                <div className="grid sm:grid-cols-2 gap-6 mb-6">
                    ...
                    <Input id="name" placeholder="Your name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} required className="rounded-xl border-slate-200 focus:border-[#fc87a7] focus:ring-[#fc87a7] flex-1" />
                    </div>
                    <div className="flex items-center gap-4">
                    <Label htmlFor="email" className="text-slate-700 whitespace-nowrap">Email</Label>
                    <Input id="email" type="email" placeholder="your@email.com" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} required className="rounded-xl border-slate-200 focus:border-[#fc87a7] focus:ring-[#fc87a7] flex-1" />
                    </div>
                </div>

                <div className="flex items-center gap-4 mb-6">
                    <Label htmlFor="subject" className="text-slate-700 whitespace-nowrap">Subject</Label>
                    <Input id="subject" placeholder="How can we help?" value={formData.subject} onChange={(e) => setFormData({ ...formData, subject: e.target.value })} required className="rounded-xl border-slate-200 focus:border-[#fc87a7] focus:ring-[#fc87a7] flex-1" />
                </div>

                <div className="space-y-2 mb-6">
                    <Label htmlFor="message" className="text-slate-700 mr-2">Message</Label>
                    <Textarea id="message" placeholder="Your message..." rows={5} value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })} required className="rounded-xl border-slate-200 focus:border-[#fc87a7] focus:ring-[#fc87a7] resize-none" />
                </div>

                <Button type="submit" disabled={isSubmitting} size="lg" className="bg-[#fc87a7] hover:bg-[#c14a75] text-white rounded-xl py-3 text-lg shadow-lg shadow-[#fc87a7]/20 transition-all hover:scale-[1.01] cursor-pointer">
                    {isSubmitting ? (
